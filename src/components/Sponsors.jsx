...
function Sponsors() {
    return (
        <div className="w-full min-h-screen relative flex flex-col items-center overflow-hidden">
            ...
            <div className="absolute top-1/2 transform -translate-y-1/2 text-white text-[10vw] md:text-[400px] font-black font-['Inter'] w-full text-center">
                sponsors
            </div>

            {/* Speakers Boxes */}
            <div className="flex justify-center flex-wrap mt-10 p-4">
                {/* Map through speakers for cleaner code */}
                {['Speaker 1', 'Speaker 2', 'Speaker 3', 'Speaker 4'].map((speaker, index) => (
                    <motion.div
                        key={index}
                        className="relative w-full sm:w-[441px] h-[321px] m-4 bg-[#ecc9f5]/40 rounded-[60px] border-8 border-white transition-transform duration-300 group"
